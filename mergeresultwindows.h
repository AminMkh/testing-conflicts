...
public:
   ...
   static std::string tags_random_postfix;
   static std::string xml_out_file;
...
private:
   ...
   /*
    * tagify Inserts tag_name into tag braces with the global hash string added as a prefix
    *
    * @param tag_name
    * @param type The type of the tag [opening/closing]
    * @param nb_indentations The number of tabs to be inserted before the tag
    *
    * @return Final tag string
    */
   inline string tagify(string tag_name, e_TagType type, int nb_indentations = 0) {
      string tabs = "";
      ...
      string lt_sym = (type == closing) ? "</" : "<";
      string gt_sym = (type == self_closing) ? "/>" : ">";

      return tabs + lt_sym + tag_name + \
             MergeResultWindow::tags_random_postfix + gt_sym;
   }
...
private:
   ...
   friend void printVersion(int version, MergeResultWindow::MergeLine& ml, MergeResultWindow *mrw, ofstream &fout);
