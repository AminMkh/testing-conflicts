...
/*
 * doesStringsHaveTags Checks if all of the given strings contain a specific key.
 *
 * @param hash String that we want to check if exists.
 * @param strs Pointer to strings array in which we will explore;
 *    Notice that this function takes care of deleting the memory allocated for strs,
 *    So it needs to be dynamically allocated.
 * @param nb_strings.
 *
 * @return Returns true if any of the strings have the hash value as a prefix to
 *    any of the tags we use in our output, false otherwise.
 */
inline bool doesStringsHaveTags(string &hash, string strs[], int nb_strings = 3) {
   const int NB_TAGS = 8;
   string tag_names[NB_TAGS] = {"data", "lines", "version_A", "version_B",
      "version_C", "conflict", "result_piece", "no_src /"};

   for (int itag = 0; itag < NB_TAGS ; ++itag) {
      string starting_braces[2] = {"<", "</"};
      string tag_to_find = hash + tag_names[itag] + ">";

      for (int istr = 0; istr < nb_strings; ++istr) {
         if (strs[istr].find(starting_braces[0] + tag_to_find) != string::npos ||
             strs[istr].find(starting_braces[1] + tag_to_find) != string::npos
         ) {
            delete[] strs;
            return true;
         }
...
/*
 * initiateRandomPrefix Returns a string (Dot ending hexadecimal hash) that can be used as a prefix
 *    to output tags without interfering with the original source code.
 *
 * @param m_sds Dynamically allocated array of source data for the files A, B and C;
 *    This function takes the responsability of freeing the memory of the sent array.
 *
 * @return The hash string in case of success, "failed_to_append" otherwise.
 */
string initiateRandomPrefix(SourceData m_sds[3])
{
   string result_hash;
   ...
   for (int i = 0; i < MAX_TRIALS; ++i) {
      ...
      for (int j = 0; j < HASH_LENGTH; ++j) {
         str_stream << hex << rand() % 16;
      }
      result_hash = string(".") + str_stream.str();

      if (!doesStringsHaveTags(result_hash, new string[3] {
         m_sds[0].getText().toUtf8().constData(),
         m_sds[1].getText().toUtf8().constData(),
         m_sds[2].getText().toUtf8().constData()
      })) {
         delete[] m_sds;
         return result_hash;
      }
...
void KDiff3App::mainInit( TotalDiffStatus* pTotalDiffStatus, bool bLoadFiles, bool bUseCurrentEncoding)
{
   ...
   MergeResultWindow::tags_random_postfix = initiateRandomPrefix(new SourceData[3]{m_sd1, m_sd2, m_sd3});
